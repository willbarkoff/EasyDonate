...
func main() {
	...
	siteFiles, err := fs.Sub(static, "client/dist")
	if err != nil {
		errors.FatalMsg(err, "An error occured while setting up the static site filesystem.")
	}

	r := server.SetupRoutes(siteFiles, db)

	srv := http.Server{}
	srv.Handler = r
	srv.Addr = ":" + strconv.Itoa(port)
	errors.Logger.Info().Int("Port", port).Msg("Starting Donorfide server")
